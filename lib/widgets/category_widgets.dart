...
class _CategoryListWidgetState extends State<CategoryListWidget> {
  ...
  List<Data> get _filtered {
    ...
    return widget.categories.where((c) {
      return (c.name ?? '').toLowerCase().contains(q) ||
          (c.skubarCode ?? '').toLowerCase().contains(q);
    }).toList();
  }
  ...
  Widget _placeholder() {
    return Container(
      ...
      child: Icon(Icons.category, color: Colors.grey.shade400, size: 26),
    );
  }

  void _confirmDelete(BuildContext context, Data cat) {
    showDialog(
      context: context,
